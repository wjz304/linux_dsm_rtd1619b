...
int xsGetBuzzerCleared(unsigned char *buzzer_cleared)
{
	...
	*buzzer_cleared = 0;
	...
	Pin.pin = ICELAKED_BUZZER_OFF_PIN;
    if ( 0 > GetGpioPin( &Pin ) ) {
#ifdef MY_DEF_HERE
		giDenoOfTimeInterval = -1;
#endif /* MY_DEF_HERE */
...
int GetModel(void)
{
	int model = MODEL_RS4023xsp;

	if (!strncmp(gszSynoHWVersion, HW_RS4023xsp, strlen(HW_RS4023xsp))) {
			model = MODEL_RS4023xsp;
	}
...
int synobios_model_init(struct file_operations *fops, struct synobios_ops **ops)
{
	...
	*ops = &synobios_ops;
	...
	{
		case MODEL_RS4023xsp:
			model_ops = &rs4023xsp_ops;
			RS4023xspSMBusSwitchInit();
			synobios_ops.set_power_led = SetPowerLedStatus;
			hwmon_sensor_list = &rs4023xsp_sensor_list;
