...
/**
 * SetDiskLedStatusByJMB585GPIOByPort - control light of slot number on JMB585
 *
 * return 0: success
 *        -EINVAL: fail
 */
int SetDiskLedStatusByJMB585GPIOByPort(DISKLEDSTATUS *pLedStatus)
{
	...
	iRet = syno_jmb585_disk_led_set(iDiskPort, iWrite);
END:
	return iRet;
}
...
int SYNOSetDiskLedStatusByJMB585GPIOByPort(int iHostNum, SYNO_DISK_LED iStatus)
{
	...
	return SetDiskLedStatusByJMB585GPIOByPort(&ledStatus);
}
