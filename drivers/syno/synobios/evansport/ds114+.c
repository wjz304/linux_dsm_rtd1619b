...
PWM_FAN_SPEED_MAPPING gDS114pSpeedMapping[] = {
...
static
int DS114pInitModuleType(struct synobios_ops *ops)
{	
	module_t type_114p = MODULE_T_DS114pv1;
	module_t *pType = &type_114p;
...
static
int DS114pFanSpeedMapping(FAN_SPEED speed)
{
	int iDutyCycle = -1;
	size_t i;

	for( i = 0; i < sizeof(gDS114pSpeedMapping)/sizeof(PWM_FAN_SPEED_MAPPING); ++i ) {
		if( gDS114pSpeedMapping[i].fanSpeed == speed ) {
			iDutyCycle = gDS114pSpeedMapping[i].iDutyCycle;
			break;
		}
	}
...
static 
int DS114pSetEsataLedStatus(SYNO_DISK_LED status)
{
	int iRet = -1;
...
struct model_ops ds114p_ops = {
	.x86_init_module_type = DS114pInitModuleType,
	.x86_fan_speed_mapping = DS114pFanSpeedMapping, 
	.x86_set_esata_led_status = DS114pSetEsataLedStatus,
