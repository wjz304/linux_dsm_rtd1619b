...
PWM_FAN_SPEED_MAPPING gDS418playSpeedMapping[] = {
...
SYNO_HWMON_SENSOR_TYPE ds418play_hdd_backplane_status = {
...
static
void DS418playGpioCleanup(void)
{
#ifdef CONFIG_SYNO_PORT_MAPPING_V2
#else
...
static
int DS418playInitModuleType(struct synobios_ops *ops)
{
	module_t type_418play = MODULE_T_DS418play;
	module_t *pType = &type_418play;
...
static
int DS418playFanSpeedMapping(FAN_SPEED speed)
{
	int iDutyCycle = -1;
	size_t i;

	for( i = 0; i < sizeof(gDS418playSpeedMapping)/sizeof(PWM_FAN_SPEED_MAPPING); ++i ) {
		if( gDS418playSpeedMapping[i].fanSpeed == speed ) {
			iDutyCycle = gDS418playSpeedMapping[i].iDutyCycle;
			break;
		}
	}
...
struct model_ops ds418play_ops = {
	.x86_init_module_type = DS418playInitModuleType,
	.x86_fan_speed_mapping = DS418playFanSpeedMapping,
	...
	.x86_gpio_init = DS418playGpioInit,
	.x86_gpio_cleanup = DS418playGpioCleanup,
};

struct hwmon_sensor_list ds418play_sensor_list = {
       .thermal_sensor = NULL,
       .voltage_sensor = NULL,
       .fan_speed_rpm = NULL,
       .psu_status = NULL,
       .hdd_backplane = &ds418play_hdd_backplane_status,
};
