...
PWM_FAN_SPEED_MAPPING gDS419pSpeedMapping[] = {
...
SYNO_HWMON_SENSOR_TYPE ds419p_hdd_backplane_status = {
...
static
int DS419pInitModuleType(struct synobios_ops *ops)
{
	module_t type_419p = MODULE_T_DS419p;
	module_t *pType = &type_419p;
...
static
int DS419pFanSpeedMapping(FAN_SPEED speed)
{
	int iDutyCycle = -1;
	size_t i;

	for( i = 0; i < sizeof(gDS419pSpeedMapping)/sizeof(PWM_FAN_SPEED_MAPPING); ++i ) {
		if( gDS419pSpeedMapping[i].fanSpeed == speed ) {
			iDutyCycle = gDS419pSpeedMapping[i].iDutyCycle;
			break;
		}
	}
...
struct model_ops ds419p_ops = {
	.x86_init_module_type = DS419pInitModuleType,
	.x86_fan_speed_mapping = DS419pFanSpeedMapping,
	...
	.x86_gpio_init = DS419pGpioInit,
	.x86_gpio_cleanup = DS419pGpioCleanup,
};

struct hwmon_sensor_list ds419p_sensor_list = {
       .thermal_sensor = NULL,
       .voltage_sensor = NULL,
       .fan_speed_rpm = NULL,
       .psu_status = NULL,
       .hdd_backplane = &ds419p_hdd_backplane_status,
};
