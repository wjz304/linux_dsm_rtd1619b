...
PWM_FAN_SPEED_MAPPING gDS918pSpeedMapping[] = {
...
SYNO_HWMON_SENSOR_TYPE ds918p_hdd_backplane_status = {
...
static
int DS918pInitModuleType(struct synobios_ops *ops)
{
	module_t type_918p = MODULE_T_DS918p;
	module_t *pType = &type_918p;
...
static
int DS918pFanSpeedMapping(FAN_SPEED speed)
{
	int iDutyCycle = -1;
	size_t i;

	for( i = 0; i < sizeof(gDS918pSpeedMapping)/sizeof(PWM_FAN_SPEED_MAPPING); ++i ) {
		if( gDS918pSpeedMapping[i].fanSpeed == speed ) {
			iDutyCycle = gDS918pSpeedMapping[i].iDutyCycle;
			break;
		}
	}
...
struct model_ops ds918p_ops = {
	.x86_init_module_type = DS918pInitModuleType,
	.x86_fan_speed_mapping = DS918pFanSpeedMapping,
	...
	.x86_gpio_init = DS918pGpioInit,
	.x86_gpio_cleanup = DS918pGpioCleanup,
};

struct hwmon_sensor_list ds918p_sensor_list = {
       .thermal_sensor = NULL,
       .voltage_sensor = NULL,
       .fan_speed_rpm = NULL,
       .psu_status = NULL,
       .hdd_backplane = &ds918p_hdd_backplane_status,
};
