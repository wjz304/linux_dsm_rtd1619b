...
PWM_FAN_SPEED_MAPPING gDS1019pSpeedMapping[] = {
...
SYNO_HWMON_SENSOR_TYPE ds1019p_hdd_backplane_status = {
...
static
int DS1019pInitModuleType(struct synobios_ops *ops)
{
	module_t type_1019p = MODULE_T_DS1019p;
	module_t *pType = &type_1019p;
...
static
int DS1019pFanSpeedMapping(FAN_SPEED speed)
{
	int iDutyCycle = -1;
	size_t i;

	for( i = 0; i < sizeof(gDS1019pSpeedMapping)/sizeof(PWM_FAN_SPEED_MAPPING); ++i ) {
		if( gDS1019pSpeedMapping[i].fanSpeed == speed ) {
			iDutyCycle = gDS1019pSpeedMapping[i].iDutyCycle;
			break;
		}
	}
...
struct model_ops ds1019p_ops = {
	.x86_init_module_type = DS1019pInitModuleType,
	.x86_fan_speed_mapping = DS1019pFanSpeedMapping,
	...
	.x86_gpio_init = DS1019pGpioInit,
	.x86_gpio_cleanup = DS1019pGpioCleanup,
};

struct hwmon_sensor_list ds1019p_sensor_list = {
       .thermal_sensor = NULL,
       .voltage_sensor = NULL,
       .fan_speed_rpm = NULL,
       .psu_status = NULL,
       .hdd_backplane = &ds1019p_hdd_backplane_status,
};
