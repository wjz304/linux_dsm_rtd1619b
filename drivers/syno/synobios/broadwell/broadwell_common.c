...
static int GetMaxInternalDiskNum(void)
{
	...
	switch(GetModel()) {
		case MODEL_DS3017xs:
			...
			break;
		case MODEL_RS3618xs:
		case MODEL_RS3617rpxs:
		case MODEL_RS3617xsp:
		case MODEL_RSD18016xsp:
		case MODEL_DS3617xs:
		case MODEL_RS18017xsp:
		case MODEL_DS3617xsII:
		case MODEL_DS3622xsp:
			iMaxInternalDiskNum = 12;
			break;
		case MODEL_RS4017xsp:
...
int xsGetBuzzerCleared(unsigned char *buzzer_cleared)
{
	...
	*buzzer_cleared = 0;
	...
	Pin.pin = BROADWELL_BUZZER_OFF_PIN;
    if ( 0 > GetGpioPin( &Pin ) ) {
#ifdef MY_DEF_HERE
		giDenoOfTimeInterval = -1;
#endif /* MY_DEF_HERE */
...
int BroadwellRedundantPowerGetPowerStatus(POWER_INFO *power_info)
{
	int err = -1;
	int power1_Value = 0, power2_Value = 0;

	if ( 0 != syno_pch_lpc_gpio_pin(BROADWELL_POWER1_PIN , &power1_Value, 0) ) {
		goto FAIL;
	}

	if ( 0 != syno_pch_lpc_gpio_pin(BROADWELL_POWER2_PIN , &power2_Value, 0) ) {
		goto FAIL;
	}
...
static
int HWMONGetPMBUSStatus(SYNO_HWMON_SENSOR_TYPE* PsuStatus, int psu_no)
{
	int iRet = -1;
	if (NULL == PsuStatus || 0 >= psu_no || NULL == hwmon_sensor_list) {
		goto END;
	}

	memcpy(PsuStatus, hwmon_sensor_list->psu_status, psu_no * sizeof(SYNO_HWMON_SENSOR_TYPE));

	iRet = syno_get_pmbus_status(PsuStatus, psu_no);
...
int GetModel(void)
{
	int model = MODEL_RSD18016xsp;

	if (!strncmp(gszSynoHWVersion, HW_RSD18016xsp, strlen(HW_RSD18016xsp))) {
			model = MODEL_RSD18016xsp;
	} else if (!strncmp(gszSynoHWVersion, HW_RS3617xsp, strlen(HW_RS3617xsp))) {
			model = MODEL_RS3617xsp;
	} else if (!strncmp(gszSynoHWVersion, HW_RS3618xs, strlen(HW_RS3618xs))) {
			model = MODEL_RS3618xs;
	} else if (!strncmp(gszSynoHWVersion, HW_RS3617rpxs, strlen(HW_RS3617rpxs))) {
			model = MODEL_RS3617rpxs;
	} else if (!strncmp(gszSynoHWVersion, HW_RS4017xsp, strlen(HW_RS4017xsp))) {
			model = MODEL_RS4017xsp;
	} else if (!strncmp(gszSynoHWVersion, HW_DS3617xs, strlen(HW_DS3617xs))) {
			model = MODEL_DS3617xs;
	} else if (!strncmp(gszSynoHWVersion, HW_RS18017xsp, strlen(HW_RS18017xsp))) {
			model = MODEL_RS18017xsp;
	} else if (!strncmp(gszSynoHWVersion, HW_FS2017, strlen(HW_FS2017))) {
			model = MODEL_FS2017;
	} else if (!strncmp(gszSynoHWVersion, HW_DS3017xs, strlen(HW_DS3017xs))) {
			model = MODEL_DS3017xs;
	} else if (!strncmp(gszSynoHWVersion, HW_FS3400, strlen(HW_FS3400))) {
			model = MODEL_FS3400;
	} else if (!strncmp(gszSynoHWVersion, HW_DS3617xsII, strlen(HW_DS3617xsII))) {
			model = MODEL_DS3617xsII;
...
int SetAlarmLed(unsigned char ledON)
{
	...
	Pin.pin = BROADWELL_ALARM_LED_PIN;
	/**
	 *	Attetion!!
	 *	0 Means alarm on
	 *	1 Means alarm off
...
#endif /* MY_DEF_HERE */
#endif /* CONFIG_SYNO_MV1475_SGPIO_LED_CTRL */
	giDiskLedController = BROADWELL_DISK_LED_ACTIVATE_PIN;

	switch(GetModel())
	{
		case MODEL_RS3618xs:
			model_ops = &rs3618xs_ops;
			if (syno_is_hw_revision(HW_R1)) {
				RS3618xsSMBusSwitchInit();
			...
			hwmon_sensor_list = &rs3618xs_sensor_list;
			...
			if (syno_is_hw_revision(HW_R1) ||
				...
				hwmon_sensor_list->hdd_backplane = &RS3618xs_hdd_backplane_status;
#ifdef CONFIG_SYNO_SMBUS_HDD_POWERCTL
			...
			break;
		case MODEL_RS3617rpxs:
			model_ops = &rs3617rpxs_ops;
			hwmon_sensor_list = &rs3617rpxs_sensor_list;
			synobios_ops.hwmon_get_fan_speed_rpm = HWMONGetFanSpeedRPMFromADT;
			synobios_ops.hwmon_get_sys_voltage = HWMONGetVoltageSensorFromADT;
			synobios_ops.hwmon_get_sys_thermal = HWMONGetThermalSensorFromADT;
			break;
		case MODEL_RS3617xsp:
			model_ops = &rs3617xsp_ops;
			hwmon_sensor_list = &rs3617xsp_sensor_list;
			synobios_ops.hwmon_get_fan_speed_rpm = HWMONGetFanSpeedRPMFromADT;
			synobios_ops.hwmon_get_sys_voltage = HWMONGetVoltageSensorFromADT;
			synobios_ops.hwmon_get_sys_thermal = HWMONGetThermalSensorFromADT;
			break;
		case MODEL_RSD18016xsp:
			model_ops = &rsd18016xsp_ops;
			break;
		case MODEL_RS4017xsp:
			model_ops = &rs4017xsp_ops;
			hwmon_sensor_list = &rs4017xsp_sensor_list;
			synobios_ops.hwmon_get_fan_speed_rpm = HWMONGetFanSpeedRPMFromADT;
			synobios_ops.hwmon_get_sys_voltage = HWMONGetVoltageSensorFromADT;
			synobios_ops.hwmon_get_sys_thermal = HWMONGetThermalSensorFromADT;
			break;
		case MODEL_DS3617xs:
			model_ops = &ds3617xs_ops;
			hwmon_sensor_list = &ds3617xs_sensor_list;
			synobios_ops.hwmon_get_fan_speed_rpm = HWMONGetFanSpeedRPMFromADT;
			synobios_ops.hwmon_get_sys_voltage = HWMONGetVoltageSensorFromADT;
			synobios_ops.hwmon_get_sys_thermal = HWMONGetThermalSensorFromADT;
			break;
		case MODEL_RS18017xsp:
			model_ops = &rs18017xsp_ops;
#ifdef CONFIG_SYNO_SAS_HOST_DISK_LED_CTRL
			if (NULL != syno_valid_lsi3008_led) {
				synobios_ops.set_hdd_led = syno_valid_lsi3008_led;
			}
#endif /* CONFIG_SYNO_SAS_HOST_DISK_LED_CTRL */
			hwmon_sensor_list = &rs18017xsp_sensor_list;
			synobios_ops.hwmon_get_fan_speed_rpm = HWMONGetFanSpeedRPMFromADT;
			synobios_ops.hwmon_get_sys_voltage = HWMONGetVoltageSensorFromADT;
			synobios_ops.hwmon_get_sys_thermal = HWMONGetThermalSensorFromADT;
			break;
		case MODEL_FS2017:
			model_ops = &fs2017_ops;
#ifdef CONFIG_SYNO_SAS_HOST_DISK_LED_CTRL
			if (NULL != syno_valid_lsi3008_led) {
				synobios_ops.set_hdd_led = syno_valid_lsi3008_led;
			}
#endif /* CONFIG_SYNO_SAS_HOST_DISK_LED_CTRL */
			hwmon_sensor_list = &fs2017_sensor_list;
			synobios_ops.hwmon_get_fan_speed_rpm = HWMONGetFanSpeedRPMFromADT;
			synobios_ops.hwmon_get_sys_voltage = HWMONGetVoltageSensorFromADT;
			synobios_ops.hwmon_get_sys_thermal = HWMONGetThermalSensorFromADT;
			break;
		case MODEL_FS3400:
			model_ops = &fs3400_ops;
#ifdef CONFIG_SYNO_SAS_HOST_DISK_LED_CTRL
			if (NULL != syno_valid_lsi3008_led) {
				synobios_ops.set_hdd_led = syno_valid_lsi3008_led;
			}
#endif /* CONFIG_SYNO_SAS_HOST_DISK_LED_CTRL */
			hwmon_sensor_list = &fs3400_sensor_list;
			synobios_ops.hwmon_get_fan_speed_rpm = HWMONGetFanSpeedRPMFromADT;
			synobios_ops.hwmon_get_sys_voltage = HWMONGetVoltageSensorFromADT;
			synobios_ops.hwmon_get_sys_thermal = HWMONGetThermalSensorFromADT;
			break;
		case MODEL_DS3017xs:
			model_ops = &ds3017xs_ops;
			...
			syno_ahci_disk_led_enable(3, 1);
			syno_ahci_disk_led_enable(13, 1);
			syno_ahci_disk_led_enable(14, 1);
#endif /* CONFIG_SYNO_AHCI_SOFTWARE_ACITIVITY */
			break;
		case MODEL_DS3617xsII:
			model_ops = &ds3617xsII_ops;
			hwmon_sensor_list = &ds3617xsII_sensor_list;
			synobios_ops.hwmon_get_fan_speed_rpm = HWMONGetFanSpeedRPMFromADT;
			synobios_ops.hwmon_get_sys_voltage = HWMONGetVoltageSensorFromADT;
			synobios_ops.hwmon_get_sys_thermal = HWMONGetThermalSensorFromADT;
			break;
