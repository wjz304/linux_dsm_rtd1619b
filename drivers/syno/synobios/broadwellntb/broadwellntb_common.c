...
int xsGetBuzzerCleared(unsigned char *buzzer_cleared)
{
	...
	*buzzer_cleared = 0;
	...
	Pin.pin = BROADWELLNTB_BUZZER_OFF_PIN;
    if ( 0 > GetGpioPin( &Pin ) ) {
#ifdef MY_DEF_HERE
		giDenoOfTimeInterval = -1;
#endif /* MY_DEF_HERE */
...
int BroadwellntbRedundantPowerGetPowerStatus(POWER_INFO *power_info)
{
	int err = -1;
	int power1_Value = 0, power2_Value = 0;

	if ( 0 != syno_pch_lpc_gpio_pin(BROADWELLNTB_POWER1_PIN , &power1_Value, 0) ) {
		goto FAIL;
	}

	if ( 0 != syno_pch_lpc_gpio_pin(BROADWELLNTB_POWER2_PIN , &power2_Value, 0) ) {
		goto FAIL;
	}
...
int GetModel(void)
{
	...
	if (!strncmp(gszSynoHWVersion, HW_TAIPEI, strlen(HW_TAIPEI))) {
			model = MODEL_TAIPEI;
	}
...
int SetAlarmLed(unsigned char ledON)
{
	...
	Pin.pin = BROADWELLNTB_ALARM_LED_PIN;
	/**
	 *	Attetion!!
	 *	0 Means alarm on
	 *	1 Means alarm off
...
static void OkToRemoveLedWork(struct work_struct *work){
	...
	Pin.pin = BROADWELLNTB_REMOTE_LED_GPIO_PIN;
...
int synobios_model_init(struct file_operations *fops, struct synobios_ops **ops)
{
	*ops = &synobios_ops;
	...
	giDiskLedController = BROADWELLNTB_DISK_LED_ACTIVATE_PIN;

	switch(GetModel())
	{
		case MODEL_TAIPEI:
			model_ops = &taipei_ops;
#ifdef CONFIG_SYNO_SAS_HOST_DISK_LED_CTRL
