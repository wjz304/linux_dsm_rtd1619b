...
#define SYNO_RTC_MVEBU_DEVICE "rtc0"

extern SYNO_AUTO_POWERON gPwSched;

int rtc_mvebu_set_time(SYNORTCTIMEPKT* pRtcTimePkt)
{
	int iRet = -1;
	struct rtc_time tm = {0};
	struct rtc_device *rtc = rtc_class_open(SYNO_RTC_MVEBU_DEVICE);

	if (rtc == NULL) {
		printk("synobios: unable to open rtc device (%s)\n", SYNO_RTC_MVEBU_DEVICE);
		goto End;
	}
...
int rtc_mvebu_get_time(SYNORTCTIMEPKT* pRtcTimePkt)
{
	int iRet = -1;
	struct rtc_time tm = {0};
	struct rtc_device *rtc = rtc_class_open(SYNO_RTC_MVEBU_DEVICE);
...
static int rtc_mvebu_set_alarm_irq_enabled(int enabled)
{
	int iRet = -1;
	struct rtc_device *rtc = rtc_class_open(SYNO_RTC_MVEBU_DEVICE);
...
{
	return rtc_mvebu_set_alarm_irq_enabled(1);
}
...
static int rtc_mvebu_rotate_auto_poweron(SYNO_AUTO_POWERON* pAutoPowerOn, const SYNORTCTIMEPKT *pRtcTime)
{
	int iRet = -1;
...
int rtc_mvebu_set_alarm(SYNO_AUTO_POWERON *pAutoPowerOn)
{
	...
	if (0 != rtc_mvebu_get_time(&rtcTime)) {
		goto End;
	}

	memcpy(&gPwSched, pAutoPowerOn, sizeof(SYNO_AUTO_POWERON));

	if (0 != rtc_mvebu_rotate_auto_poweron(pAutoPowerOn, &rtcTime)) {
