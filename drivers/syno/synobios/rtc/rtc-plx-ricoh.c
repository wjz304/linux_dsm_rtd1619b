...
int rtc_ricoh_rotate_auto_poweron(SYNO_AUTO_POWERON* pAutoPowerOn, const SYNORTCTIMEPKT *pRtcTime)
{
	...
	if ( 0 > (ret = plxI2CCharRead(I2C_RTC_ADDR, (u8 *)&csr, sizeof(csr)/sizeof(u8), I2C_RTC_CONTROL1_OFFSET)) ) {
		goto End;
	}
    ...
    if ( SYNO_AUTO_POWERON_DISABLE == pAutoPowerOn->enabled ) {
		...
		if ( 0 > (ret = plxI2CCharWrite(I2C_RTC_ADDR, (u8 *)&csr, sizeof(csr)/sizeof(u8), I2C_RTC_CONTROL1_OFFSET)) ) {
			goto End;
		}
    } else {
		...
		csr = 0;
		if ( 0 > (ret = plxI2CCharWrite(I2C_RTC_ADDR, (u8 *)&csr, sizeof(csr)/sizeof(u8), I2C_RTC_CONTROL2_OFFSET)) ) {
			goto End;
		}
		
		// set Alarm data
		if( 0 > (ret = plxI2CCharWrite(I2C_RTC_ADDR, (u8 *)&alarmTime,
...
int rtc_ricoh_get_time(struct _SynoRtcTimePkt *pRtcTimePkt)
{
	...
	if ( 0 != (ret = plxI2CCharRead(0x32, (u8 *)rgRtcTimeTemp, 7, 0)) ) {
		ret = -1;
		goto END;
	}
...
int rtc_ricoh_set_time(struct _SynoRtcTimePkt *pRtcTimePkt)
{
	...
	if ( 0 != (ret = plxI2CCharWrite(0x32, (u8 *)rgRtcTimeTemp, 7, 0)) ) {
		ret = -1;
	}
...
void rtc_ricoh_time_correction_set(void)
{
	...
	plxI2CCharWrite(I2C_RTC_ADDR, (u8 *)&correction, 1, I2C_RTC_TIME_TRIM_OFFSET);
}

void rtc_ricoh_time_correction_get(void)
{
	unsigned char correction = 0;
	plxI2CCharRead(I2C_RTC_ADDR, &correction, 1, I2C_RTC_TIME_TRIM_OFFSET);
...
int rtc_ricoh_auto_poweron_init(void)
{
	...
	if ( 0 > (err = plxI2CCharWrite(I2C_RTC_ADDR, (u8 *)&csr, sizeof(csr)/sizeof(u8), I2C_RTC_CONTROL2_OFFSET)) ) {
		goto End;
	}
