...
static SYNO_HWMON_SENSOR_TYPE hdd_backplane_status_detect_enable = {
...
static struct hwmon_sensor_list hdd_detect_enable_sensor_list = {
	.thermal_sensor = NULL,
	.voltage_sensor = NULL,
	.fan_speed_rpm = NULL,
	.psu_status = NULL,
	.hdd_backplane = &hdd_backplane_status_detect_enable,
};
...
#endif /* CONFIG_SYNO_PORT_MAPPING_V2 */
		if (MODEL_RS819 == GetModel() && 2 == index) {
			// rs819 HDD 2 is always power on and power pin is fake.
			hdd_enable |= 0x01 << (index - 1);
		}
...
int SetPhyLedR8169NoCtrlPin(SYNO_LED status)
{
	...
	for_each_netdev(&init_net, dev) {
		if (dev->ethtool_ops && dev->ethtool_ops->get_drvinfo) {
			...
			if (!strcmp(info.driver, "r8169")) {
				syno_rtd129x_rtl8169_lan_led_control(dev, val);
				err = 0;
				break;
			}
		}
...
static struct synobios_ops synobios_ops = {
	...
	.get_brand            = GetBrand,
	.get_model            = GetModel,
	.get_rtc_time         = rtc_rtk_get_time,
	.set_rtc_time         = rtc_rtk_set_time,
	.get_fan_status       = NULL,
	...
	.get_auto_poweron     = rtc_rtk_get_alarm,
	.set_auto_poweron     = rtc_rtk_set_alarm,
	.init_auto_poweron    = rtc_rtk_enable_alarm_irq,
	.uninit_auto_poweron  = rtc_rtk_enable_alarm_irq,
	.set_alarm_led        = NULL,
...
int synobios_model_init(struct file_operations *fops, struct synobios_ops **ops)
{
	...
	switch (GetModel()) {
		...
		case MODEL_DS220j:
		case MODEL_DS420j:
			hwmon_sensor_list = &hdd_detect_enable_sensor_list;
			break;
		case MODEL_EDS19:
