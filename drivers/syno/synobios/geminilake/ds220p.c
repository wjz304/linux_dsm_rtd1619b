...
PWM_FAN_SPEED_MAPPING gDS220pSpeedMapping[] = {
...
SYNO_HWMON_SENSOR_TYPE ds220p_hdd_backplane_status = {
...
SYNO_HWMON_SENSOR_TYPE ds220p_current_status = {
...
static
int DS220pInitModuleType(struct synobios_ops *ops)
{
	module_t type_220p = MODULE_T_DS220p;
	module_t *pType = &type_220p;
...
static
int DS220pFanSpeedMapping(FAN_SPEED speed)
{
	int iDutyCycle = -1;
	size_t i;

	for( i = 0; i < sizeof(gDS220pSpeedMapping)/sizeof(PWM_FAN_SPEED_MAPPING); ++i ) {
		if( gDS220pSpeedMapping[i].fanSpeed == speed ) {
			iDutyCycle = gDS220pSpeedMapping[i].iDutyCycle;
			break;
		}
	}
...
struct model_ops ds220p_ops = {
	.x86_init_module_type = DS220pInitModuleType,
	.x86_fan_speed_mapping = DS220pFanSpeedMapping,
	...
	.x86_gpio_init = DS220pGpioInit,
	.x86_gpio_cleanup = DS220pGpioCleanup,
};

struct hwmon_sensor_list ds220p_sensor_list = {
       .thermal_sensor = NULL,
       .voltage_sensor = NULL,
       .fan_speed_rpm = NULL,
       .psu_status = NULL,
       .hdd_backplane = &ds220p_hdd_backplane_status,
       .current_sensor = &ds220p_current_status,
};

#if defined(CONFIG_SYNO_SYSTEM_SHUTDOWN_HOOK) && defined(CONFIG_SYNO_GPIO)
static void DS220pLedDisable(void)
{
...
SYNO_SHUTDOWN_HOOK ds220p_shutdown_hook = {
	.shutdown = DS220pLedDisable,
};
